...
export const useBuilderStore = create<BuilderState>((set, get) => {
  ...
  return { dashboard: emptyDashboard, history:[], future:[], filterValues:{}, preview:false, dark:false, viewport:'desktop', jsonOpen:false, dataOpen:false,
    ...
    updatePosition: (id, position, viewport = 'desktop') => { const state=get(); push({...state.dashboard,widgets:state.dashboard.widgets.map(w => w.id===id ? viewport === 'desktop' ? {...w,position} : {...w,positions:{...w.positions,[viewport]:position}} : w)}); },
