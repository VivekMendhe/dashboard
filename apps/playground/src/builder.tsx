...
import { getWidget, listWidgets, type DashboardConfig, type DashboardWidget, type DataRecord, type FilterConfig, type WidgetRenderProps } from '@dashboard-generator/core';
...
function WidgetContent({ widget }: { widget: DashboardWidget }) {
  ...
  const [loading, setLoading] = useState(Boolean(widget.datasource));
  ...
  useEffect(() => { let active = true; loadData(widget.datasource).then((result) => { if (active) setData(result.data); }).catch(() => active && setData([])).finally(() => active && setLoading(false)); return () => { active = false; }; }, [widget.datasource]);
  if (!definition) return <div className="pg-empty">Unknown widget</div>;
  const props: WidgetRenderProps = { widget, data, loading, filters, theme: dark ? darkTheme : lightTheme };
...
function Canvas() {
  ...
  const layout = useMemo<Layout>(() => dashboard.widgets.map((widget) => ({ i: widget.id, ...widget.position, minW: widget.position.minW ?? 2, maxW: widget.position.maxW ?? 12, minH: widget.position.minH ?? 2, maxH: widget.position.maxH ?? 12, static: Boolean(widget.options?.locked) })), [dashboard.widgets]);
  const commit = (items: Layout) => items.forEach((item) => { const before = dashboard.widgets.find((widget) => widget.id === item.i)?.position; if (before && (before.x !== item.x || before.y !== item.y || before.w !== item.w || before.h !== item.h)) updatePosition(item.i, { x: item.x, y: item.y, w: item.w, h: item.h, minW: before.minW, minH: before.minH, maxW: before.maxW, maxH: before.maxH }); });
